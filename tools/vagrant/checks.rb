...
def check_aws_needs()
  ...
  if !(ENV['CL_AWS_ACCESS_KEY'] and ENV['CL_AWS_SECRET_KEY'] \
        and ENV['CL_AWS_KEY_NAME'] and ENV['CL_AWS_KEY_PATH'])
    abort 'If you want to use the AWS provider, you must provide these four variables: 
    CL_AWS_ACCESS_KEY : Your Amazon Web Services Access Key
    CL_AWS_SECRET_KEY : Your Amazon Web Services Secret Access Key
    CL_AWS_KEY_NAME   : The name of the keypair you are using
    CL_AWS_KEY_PATH   : Path to the *.pem file for the keypair you are using'
  end
end
...
def check_vb_needs()
  ...
  warning = "\033[31m\
WARNING: Clasp only officially supports a 64-bit 
virtual machine, which your current system may not be able to 
support. Use `CL_SHOW_VM=true vagrant up` to watch the VM launch -
...
Or forcing Clasp to attempt a 32-bit VM
   $ CL_VM_ARCH=32 vagrant up
